...
internal sealed partial class PolicyExpressionCompiler
{
        ...
        IReadOnlyDictionary<ISymbol, string>? renames = null) : CSharpSyntaxRewriter
    {
        ...
        public override SyntaxNode? VisitMemberBindingExpression(MemberBindingExpressionSyntax node) =>
            IsObsoleteUsername(model.GetSymbolInfo(node).Symbol)
                ? node.WithName(SyntaxFactory.IdentifierName("UserId").WithTriviaFrom(node.Name))
        ...
        // BasicAuthCredentials.Username is the toolkit's obsolete name for API Management's UserId.
        private static bool IsObsoleteUsername(ISymbol? symbol) =>
            symbol is IPropertySymbol { Name: "Username", ContainingType: { Name: nameof(BasicAuthCredentials) } type } &&
            IsAuthoringType(type);
        ...
        public override SyntaxNode? VisitMemberAccessExpression(MemberAccessExpressionSyntax node)
        {
            ...
            var symbol = model.GetSymbolInfo(node).Symbol;
            if (IsObsoleteUsername(symbol))
            {
                return ((MemberAccessExpressionSyntax)base.VisitMemberAccessExpression(node)!)
                    .WithName(SyntaxFactory.IdentifierName("UserId").WithTriviaFrom(node.Name));
            }
