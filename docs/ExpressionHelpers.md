...
API Management names the basic authentication user `UserId`. `BasicAuthCredentials.Username` is obsolete and compiles
to `UserId`. `UserId` defaults to `Username`, so existing `BasicAuthCredentials` implementations keep working.
